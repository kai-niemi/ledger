...
fn_create_cluster() {
  if [ "${cloud}" = "aws" ]; then
    fn_failcheck roachprod create $CLUSTER --clouds=aws --aws-machine-type-ssd=${machinetypes} \
    --aws-zones=${zones} --aws-profile crl-revenue --aws-config ~/rev.json  \
    --geo --local-ssd --nodes=${nodes} --os-volume-size 750 --lifetime 24h0m0s
  elif [ "${cloud}" = "gce" ]; then
    fn_failcheck roachprod create $CLUSTER --clouds=gce --gce-machine-type=${machinetypes} \
    --gce-zones=${zones} --aws-profile crl-revenue --aws-config ~/rev.json \
    --geo --local-ssd --nodes=${nodes} --os-volume-size 750 --lifetime 24h0m0s
  else
    fn_failcheck roachprod create $CLUSTER --clouds=azure --azure-machine-type=${machinetypes} \
    --azure-locations=${zones} --aws-profile crl-revenue --aws-config ~/rev.json \
    --geo --local-ssd --nodes=${nodes} --os-volume-size 750 --lifetime 24h0m0s
  fi
}
